...
void Demo::Render() {
	...
	glUniform3f(glGetUniformLocation(this->shadowmapShader, "dirLight.diffuse"), 0.5f, 0.5f, 0.5f);
	glUniform3f(glGetUniformLocation(this->shadowmapShader, "dirLight.specular"), 0.5f, 0.5f, 0.5f);
	// point light 1
...
void Demo::BuildTexturedCube()
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("metal2HitamBukanPutih.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
	...
	float vertices[] = {
		...
		// front
		-2.0, -0.5, 0.5, 0, 0, 0.0f,  0.0f,  1.0f, // 0
		2.0, -0.5, 0.5, 1, 0,  0.0f,  0.0f,  1.0f, // 1
		2.0,  2.0, 0.5, 1, 1,  0.0f,  0.0f,  1.0f, // 2
		-2.0,  2.0, 0.5, 0, 1, 0.0f,  0.0f,  1.0f, // 3

		// back
		-2.0, -0.5, -0.4, 0, 0, 0.0f,  0.0f,  -1.0f, // 8 
		2.0,  -0.5, -0.4, 1, 0, 0.0f,  0.0f,  -1.0f, // 9
		2.0,   2.0, -0.4, 1, 1, 0.0f,  0.0f,  -1.0f, // 10
		-2.0,  2.0, -0.4, 0, 1, 0.0f,  0.0f,  -1.0f, // 11
...
void Demo::BuildTexturedCube2()
{
	...
	float vertices[] = {
		// format position, tex coords
		// front
		-1.5, -0.1, 0.52, 0, 0, 0.0f,  0.0f,  1.0f, // 0
		1.5, -0.1, 0.52, 1, 0,  0.0f,  0.0f,  1.0f, // 1
		1.5,  1.5, 0.52, 1, 1,  0.0f,  0.0f,  1.0f, // 2
		-1.5,  1.5, 0.52, 0, 1, 0.0f,  0.0f,  1.0f, // 3
...
void Demo::BuildTexturedCube3()
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("metal2PutihBukanHitam.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
	...
	float vertices[] = {
		// format position, tex coords
		 ...
		 // right
		2.0,  2.0,  0.5, 0, 0, 1.0f,  0.0f,  0.0f, // 4
		2.0,  2.0, -0.4, 1, 0, 1.0f,  0.0f,  0.0f, // 5
		2.0, -0.5,  -0.4, 1, 1, 1.0f,  0.0f,  0.0f, // 6
		2.0, -0.5,  0.5, 0, 1, 1.0f,  0.0f,  0.0f, // 7


		 // left
		-2.0, -0.5, -0.4, 0, 0, -1.0f,  0.0f,  0.0f, // 12
		-2.0, -0.5,  0.5, 1, 0, -1.0f,  0.0f,  0.0f, // 13
		-2.0,  2.0,  0.5, 1, 1, -1.0f,  0.0f,  0.0f, // 14
		-2.0,  2.0, -0.4, 0, 1, -1.0f,  0.0f,  0.0f, // 15

		// upper
		2.0, 2.0,  0.5, 0, 0,   0.0f,  1.0f,  0.0f, // 16
		-2.0, 2.0,  0.5, 1, 0,   0.0f,  1.0f,  0.0f, // 17
		-1.5, 1.5, -0.4, 1, 1,  0.0f,  1.0f,  0.0f, // 18
		1.5, 1.5, -0.4, 0, 1,   0.0f,  1.0f,  0.0f, // 19

		// bottom
		-2.0, -0.5, -0.4, 0, 0, 0.0f,  -1.0f,  0.0f, // 20
		2.0, -0.5, -0.4, 1, 0,  0.0f,  -1.0f,  0.0f, // 21
		2.0, -0.5,  0.5, 1, 1,  0.0f,  -1.0f,  0.0f, // 22
		-2.0, -0.5,  0.5, 0, 1, 0.0f,  -1.0f,  0.0f, // 23
	};
...
void Demo::BuildTexturedPlane()
{
	...
	glGenTextures(1, &stexturePlane);
	glBindTexture(GL_TEXTURE_2D, stexturePlane);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("KayouKinclong.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
